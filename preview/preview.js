...
let compact=false, pinned=true, weekly=true;
...
function details(a){let content='';if(a.banked!==undefined){content+=`<span class="bank-hover" tabindex="0" aria-label="${a.banked} banked resets; hover or focus for expiration dates"><span class="detail-title">${icon('reserve')}${a.banked} ${a.banked===1?'reset':'resets'} banked</span><span class="bank-popover" role="tooltip">`;
...
if(a.banked!==undefined)content+='</span></span>';
...
function render(){frame.hidden=false;frame.className='frame'+(compact?' compact':'');document.querySelector('#mode').textContent=compact?'Show expanded':'Show compact';try{localStorage.setItem('agent-usage-design',JSON.stringify({compact,weekly}));}catch{}
...
if(compact){frame.innerHTML=`<div class="compact-controls"><button data-action="${weekly?'hourly':'weekly'}" aria-label="${weekly?'Show 5-hour usage':'Show weekly usage'}" title="${weekly?'Show 5-hour usage':'Show weekly usage'}">${weekly?'W':'5h'}</button>${button('fold','Expand','expand')}</div>`+`<div class="cells">${rows.map(a=>`<div class="cell" title="${a.note||a.error||((weekly?'Weekly':'5-hour')+': resets in '+a.resets[weekly?1:0])}">${gauge(a)}${identity(a)}</div>`).join('')}</div>`;return;}
